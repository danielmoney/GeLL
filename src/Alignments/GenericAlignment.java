...
public class GenericAlignment
{
     ...
     * One sequence / taxa per line - taxa name followed by the sequence.  Name is
     * separated from sequence by whitespace.  Sites in the sequence are separated
     * by white space.  Allows the use of states that are multiple characters long
     * (e.g. codon).  Blank lines are ignored.  A taxa name of <code>*class*</code> is
	...
	BufferedReader in;
        ...
        try
        {
            ...
            for (int i = 0; i < seqLength; i++)
            {
                ...
                {
                    col.put(e.getKey(),e.getValue()[i+1]);
                }
                String c = null;
                if (classLine != null)
                {
